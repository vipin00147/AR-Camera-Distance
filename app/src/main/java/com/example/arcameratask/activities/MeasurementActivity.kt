...
import com.example.arcameratask.databinding.FragmentMeasurementBinding
import com.example.arcameratask.utils.AppConstants
...
class MeasurementActivity<T> : BaseActivity<FragmentMeasurementBinding>(), Scene.OnUpdateListener {
    ...
    private val multipleDistances = Array(AppConstants.maxNumMultiplePoints,
        {Array<TextView?>(AppConstants.maxNumMultiplePoints){null} })
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arFragment = this.supportFragmentManager.findFragmentById(R.id.sceneform_fragment) as ArFragment?
    ...
    override fun createBinding(): FragmentMeasurementBinding {
        return FragmentMeasurementBinding.inflate(layoutInflater)
    }


    @SuppressLint("SetTextI18n")
    private fun initDistanceTable(){
        for (i in 0 until AppConstants.maxNumMultiplePoints+1){
            val tableRow = TableRow(this)
            for (j in 0 until AppConstants.maxNumMultiplePoints+1){
                ...
                tableRow.addView(textView,
                    tableRow.layoutParams.width / (AppConstants.maxNumMultiplePoints + 1),
    ...
    private fun clearAllAnchors(){
        ...
        for (i in 0 until AppConstants.maxNumMultiplePoints){
            for (j in 0 until AppConstants.maxNumMultiplePoints){
    ...
    private fun tapDistanceOfMultiplePoints(hitResult: HitResult){
        if (placedAnchorNodes.size >= AppConstants.maxNumMultiplePoints){
            clearAllAnchors()
        }
        ViewRenderable
            .builder()
